...
import { getAllMedicines, deleteMedicine } from "@/lib/realtime-database-service-medicines"
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (selectedMedicine) {
        ...
        await import("@/lib/realtime-database-service-medicines").then((module) => {
      ...
      toast({
        title: "Error",
        description: selectedMedicine ? "Failed to update medicine" : "Failed to add medicine",
        variant: "destructive",
      })
    } finally {
                ...
                {filteredMedicines?.map((medicine) => (
                  <TableRow key={medicine.id}>
                    ...
                    <TableCell className="hidden md:table-cell">
                      ...
                      {medicine.defaultDuration.months > 0 && `${medicine.defaultDuration.months} months `}
                      {medicine.defaultDuration.years > 0 && `${medicine.defaultDuration.years} years`}
                    </TableCell>
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-2">
                        <Button variant="ghost" size="icon" onClick={() => handleOpenDialog(medicine)}>
                          <Edit className="h-4 w-4" />
                          <span className="sr-only">Edit</span>
                        </Button>
                        <Button variant="ghost" size="icon" onClick={() => handleOpenDeleteDialog(medicine)}>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-[600px]">
          <DialogHeader>
            ...
            <DialogDescription>
              {selectedMedicine ? "Update the details of this medicine" : "Enter the details of the new medicine"}
            </DialogDescription>
          </DialogHeader>
                    ...
                    return (
                      <div key={timing} className="space-y-2">
                        <div className="flex items-center space-x-2">
                          ...
                          />
                          <Label htmlFor={`timing-${timing}`} className="text-sm font-normal">
                            {timing}
                          </Label>
                        </div>
                        ...
                        {isChecked && (
                          <div className="pl-6 space-y-2">
                            <div className="space-y-1">
                              <Label htmlFor={`quantity-${timing}`} className="text-xs">
                                Quantity
                              </Label>
                              <Input
                                id={`quantity-${timing}`}
                            ...
                            <div className="space-y-1">
                              <Label htmlFor={`instructions-${timing}`} className="text-xs">
                                Instructions
                              </Label>
                              <Select
                              ...
                              >
                                <SelectTrigger id={`instructions-${timing}`} className="h-8 text-sm">
      ...
      <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete the medicine "{selectedMedicine?.name}". This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
